...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      ...
      html: `<p><strong>${clientName}</strong> (${clientEmail})<br>Service: ${serviceName}<br>Barber: ${barberName}<br>Date: ${date} at ${time}<br>Ref: ${bookingRef}</p>`
    })
  });
