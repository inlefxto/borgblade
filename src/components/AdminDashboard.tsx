...
export default function AdminDashboard() {
  ...
  const [analyticsPickedWeek, setAnalyticsPickedWeek] = useState('');
  ...
  const fetchAnalytics = useCallback(async (from?: string, to?: string) => {
    ...
    const { data } = await supabase
      .from('bookings')
      .select('id, booking_date, time_slot, status, staff_id, client_name, client_email, booking_ref, services(name, price, duration_mins), staff(name)')
  ...
  const applyCustomPeriod = () => {
    if (analyticsPeriod === 'pick-month' && analyticsPickedMonth) {
      const [y, m] = analyticsPickedMonth.split('-').map(Number);
      const from = `${y}-${String(m).padStart(2,'0')}-01`;
      const lastDay = new Date(y, m, 0).getDate();
      const to = `${y}-${String(m).padStart(2,'0')}-${String(lastDay).padStart(2,'0')}`;
      fetchAnalytics(from, to);
    } else if (analyticsPeriod === 'pick-week' && analyticsPickedWeek) {
      const { from, to } = weekInputToRange(analyticsPickedWeek);
      if (from && to) fetchAnalytics(from, to);
    } else if (analyticsPeriod === 'range' && analyticsRangeFrom && analyticsRangeTo) {
      fetchAnalytics(analyticsRangeFrom, analyticsRangeTo);
    }
  };

  const downloadCSV = () => {
    const headers = [
      'Date', 'Time', 'Booking Ref', 'Client Name', 'Client Email',
      'Service', 'Barber', 'Duration (mins)', 'Price (€)', 'Status',
    ];
    const rows = analyticsData.map(b => [
      b.booking_date,
      ...
      b.services?.name ?? '',
      b.staff?.name ?? '',
      b.services?.duration_mins ?? '',
      (b.services?.price ?? 0).toFixed(2),
      b.status ?? '',
    ]);
    ...
    link.href = url;
    const label = analyticsPeriod === 'pick-month' ? analyticsPickedMonth
      : analyticsPeriod === 'range' ? `${analyticsRangeFrom}-to-${analyticsRangeTo}`
      : analyticsPeriod;
    link.download = `borgblade-bookings-${label}.csv`;
    link.click();
      ...
      {activeView === 'clients' ? (
        ...
        <div className="admin-main" style={{ maxWidth: 1360, margin: '0 auto', padding: '20px 32px' }}>
          {(() => {
            ...
            const getStart = (period: typeof analyticsPeriod) => {
            ...
            const filtered = start
              ? analyticsData.filter(b => b.booking_date >= start && b.booking_date <= todayStr())
              : analyticsData;
            ...
            const completedRev  = completed.reduce((s, b) => s + (b.services?.price ?? 0), 0);
            const confirmedRev  = confirmed.reduce((s, b) => s + (b.services?.price ?? 0), 0);
            const totalBookings = filtered.filter(b => b.status !== 'cancelled').length;

            const barberStats = BARBERS.map(barber => {
              const bb = filtered.filter(b => b.staff_id === barber.id);
              return {
                name:      barber.name,
                total:     bb.filter(b => b.status !== 'cancelled').length,
                completed: bb.filter(b => b.status === 'completed').length,
                confirmed: bb.filter(b => b.status === 'confirmed').length,
                cancelled: bb.filter(b => b.status === 'cancelled').length,
                revenue:   bb.filter(b => b.status === 'completed').reduce((s, b) => s + (b.services?.price ?? 0), 0),
              };
            });

            const serviceCounts: Record<string, { count: number; revenue: number }> = {};
            filtered.filter(b => b.status !== 'cancelled' && b.services?.name).forEach(b => {
              const n = b.services!.name;
              if (!serviceCounts[n]) serviceCounts[n] = { count: 0, revenue: 0 };
              serviceCounts[n].count++;
              if (b.status === 'completed') serviceCounts[n].revenue += b.services!.price;
            });
            const topServices = Object.entries(serviceCounts)
              .sort((a, b) => b[1].count - a[1].count).slice(0, 6);

            const customPeriod = ['pick-month', 'pick-week', 'range'].includes(analyticsPeriod);
            const canApply =
              (analyticsPeriod === 'pick-month' && !!analyticsPickedMonth) ||
              (analyticsPeriod === 'pick-week'  && !!analyticsPickedWeek)  ||
              (analyticsPeriod === 'range'      && !!analyticsRangeFrom && !!analyticsRangeTo);
            ...
            return (
              <>
                {/* ── Period selector ── */}
                <div style={{ background: '#0c0c0c', border: '1px solid #181818', padding: '16px 20px', marginBottom: 20 }}>
                  <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: customPeriod ? 14 : 0 }}>
                    {([
                      { key: 'today',      label: 'Today' },
                      { key: 'week',       label: 'This Week' },
                      { key: 'month',      label: 'This Month' },
                      { key: 'pick-month', label: 'Pick a Month' },
                      { key: 'pick-week',  label: 'Pick a Week' },
                      ...
                      <button
                        ...
                        onClick={() => {
                          ...
                          if (!['pick-month', 'pick-week', 'range'].includes(p.key)) {
                            fetchAnalytics();
                          }
                        }}
                        style={{
                          ...
                          background: analyticsPeriod === p.key ? '#C9A84C' : 'none',
                          border: analyticsPeriod === p.key ? 'none' : '1px solid #1e1e1e',
                          color: analyticsPeriod === p.key ? '#0A0A0A' : '#555',
                        }}
                      >{p.label}</button>
                    ))}
                    <button
                      ...
                      style={{
                        ...
                        cursor: analyticsData.length > 0 ? 'pointer' : 'not-allowed',
                        fontSize: '0.7rem', letterSpacing: '0.08em',
                        fontFamily: 'DM Sans, sans-serif', transition: 'all 0.15s',
                        background: analyticsData.length > 0 ? 'rgba(74,222,128,0.08)' : 'none',
                        border: analyticsData.length > 0 ? '1px solid #4ade8033' : '1px solid #1e1e1e',
                        color: analyticsData.length > 0 ? '#4ade80' : '#333',
                        opacity: analyticsData.length > 0 ? 1 : 0.4,
                      }}
                    >↓ Download CSV</button>
                  </div>

                  {customPeriod && (
                    <div style={{ display: 'flex', gap: 10, alignItems: 'flex-end', flexWrap: 'wrap', paddingTop: 12, borderTop: '1px solid #181818' }}>
                      {analyticsPeriod === 'pick-month' && (
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
                          <label style={{ fontSize: '0.6rem', color: '#C9A84C', letterSpacing: '0.12em', textTransform: 'uppercase' }}>Month</label>
                          <input type="month" value={analyticsPickedMonth} onChange={e => setAnalyticsPickedMonth(e.target.value)} style={ctrlInput} />
                        </div>
                      )}
                      {analyticsPeriod === 'pick-week' && (
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
                          <label style={{ fontSize: '0.6rem', color: '#C9A84C', letterSpacing: '0.12em', textTransform: 'uppercase' }}>Week</label>
                          <input type="week" value={analyticsPickedWeek} onChange={e => setAnalyticsPickedWeek(e.target.value)} style={ctrlInput} />
                        </div>
                      )}
                      {analyticsPeriod === 'range' && (
                        <>
                          <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
                            <label style={{ fontSize: '0.6rem', color: '#C9A84C', letterSpacing: '0.12em', textTransform: 'uppercase' }}>From</label>
                            <input type="date" value={analyticsRangeFrom} onChange={e => setAnalyticsRangeFrom(e.target.value)} style={ctrlInput} />
                          </div>
                          <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
                            <label style={{ fontSize: '0.6rem', color: '#C9A84C', letterSpacing: '0.12em', textTransform: 'uppercase' }}>To</label>
                            <input type="date" value={analyticsRangeTo} min={analyticsRangeFrom} onChange={e => setAnalyticsRangeTo(e.target.value)} style={ctrlInput} />
                          </div>
                          {analyticsRangeFrom && analyticsRangeTo && (
                            <div style={{ fontSize: '0.72rem', color: '#555', alignSelf: 'center' }}>
                              {analyticsRangeFrom} → {analyticsRangeTo}
                            </div>
                          )}
                        </>
                      ...
                      <button
                        ...
                        disabled={!canApply || loadingAnalytics}
                        style={{
                          padding: '6px 20px',
                          background: canApply ? '#C9A84C' : '#1e1e1e',
                          border: 'none',
                          color: canApply ? '#0A0A0A' : '#444',
                  ...
                  <>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12, marginBottom: 20 }}>
                      {[
                        ...
                        { label: 'Completed Revenue', value: `€${completedRev.toFixed(2)}`,   sub: `${completed.length} completed`,     color: '#4ade80' },
                        { label: 'Expected Revenue',  value: `€${confirmedRev.toFixed(2)}`,   sub: `${confirmed.length} confirmed`,     color: '#C9A84C' },
                        { label: 'No Shows',          value: noShows.length,                  sub: `${cancelled.length} cancellations`, color: '#f87171' },
                      ].map(card => (
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                      <div style={{ background: '#0c0c0c', border: '1px solid #181818' }}>
                        ...
                        <div style={{ padding: '8px 20px 16px' }}>
                          <div style={{ display: 'grid', gridTemplateColumns: '1fr 50px 50px 50px 70px', gap: 8, padding: '8px 0', borderBottom: '1px solid #1a1a1a', marginBottom: 4 }}>
                            {['Barber','Total','Done','Cancel','Revenue'].map(h => (
                          ...
                          {barberStats.map(b => (
                            <div key={b.name} style={{ display: 'grid', gridTemplateColumns: '1fr 50px 50px 50px 70px', gap: 8, padding: '10px 0', borderBottom: '1px solid #161616', alignItems: 'center' }}>
                              <div style={{ ...cellStyle, color: '#F2F2F2' }}>{b.name.split(' ')[0]}</div>
                              <div style={{ ...cellStyle, color: '#F2F2F2', textAlign: 'right' }}>{b.total}</div>
                              <div style={{ ...cellStyle, color: '#4ade80', textAlign: 'right' }}>{b.completed}</div>
                              <div style={{ ...cellStyle, color: '#f87171', textAlign: 'right' }}>{b.cancelled}</div>
                          ...
                          {topServices.length === 0 ? (
                            ...
                            <div key={name} style={{ display: 'grid', gridTemplateColumns: '1fr 50px 70px', gap: 8, padding: '10px 0', borderBottom: '1px solid #161616', alignItems: 'center' }}>
                              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                ...
                                <span style={{ ...cellStyle, color: '#F2F2F2' }}>{name}</span>
                              </div>
                              <div style={{ ...cellStyle, color: '#F2F2F2', textAlign: 'right' }}>{stats.count}</div>
